...
const AddShiftForm = ({ onAddShift }) => {
  ...
  const handleDropdownChange = (id, value) => {
    if (value) {
      const selectedUser = sortedUsers.find((user) => user.firstName + ' ' + user.lastName === value);
      const selectedPosition = sortedPositions.find((position) => position.jobTitle === value);
  ...
  return (
    <>
      ...
      <Button variant="primary" onClick={handleShow}>
        <AiOutlinePlus />
        <span className='ms-1'>Add Shift</span>
      </Button>
